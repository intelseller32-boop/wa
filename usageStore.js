...
const memoryStore = {}; // accountId -> { messages, actions, reportedMessages, reportedActions }

function memRow(accountId) {
  if (!memoryStore[accountId]) {
    memoryStore[accountId] = { messages: 0, actions: 0, reportedMessages: 0, reportedActions: 0 };
  }
  return memoryStore[accountId];
}

async function increment(accountId, { messages = 0, actions = 0 } = {}) {
  if (!messages && !actions) return;
...
async function peek(accountId) {
  if (!isConfigured()) {
    const row = memRow(accountId);
    return { messages: row.messages - row.reportedMessages, actions: row.actions - row.reportedActions };
  }

  const pool = getPool();
  const [rows] = await pool.query(
    "SELECT messages_sent, actions_count, reported_messages, reported_actions FROM usage_counters WHERE account_id = ?",
    [accountId]
  );
  if (!rows.length) return { messages: 0, actions: 0 };
  const row = rows[0];
  return {
...
async function pull(accountId) {
  if (!isConfigured()) {
    const row = memRow(accountId);
    const delta = { messages: row.messages - row.reportedMessages, actions: row.actions - row.reportedActions };
  ...
  if (delta.messages > 0 || delta.actions > 0) {
    await pool.query(
      "UPDATE usage_counters SET reported_messages = messages_sent, reported_actions = actions_count WHERE account_id = ?",
      [accountId]
    );
  }
  return delta;
